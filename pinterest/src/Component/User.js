...
function UserPage() {
    ...
    if (!checkUser) {
      ...
      const fetchAlbums = async () => {
        try {
          ...
          setAlbums(
            albumResponse.data?.filter((a) => a.userId == checkUser.userId)
          );
      ...
      const fetchPhotos = async () => {
        try {
          const resPhoto = await axios.get(
            `http://localhost:9999/photos?albumId=${selectedAlbumId}`
          );
  ...
  const validateForm = () => {
    ...
    if (!editedUser.address.zipCode)
      formErrors.zipCode = "Zip code is required";
    if (!editedUser.account.password)
  ...
  const handleSave = () => {
    if (validateForm()) {
      axios
        .put(`http://localhost:9999/users/${user.id}`, editedUser)
        .then(() => {
          setUser(editedUser);
  ...
  const handleAddAlbum = () => {
    ...
    if (isValid) {
      ...
      axios
        .post("http://localhost:9999/albums", newAlbum)
        .then(() => {
  ...
  const handleEditAlbumClick = (album) => {
    setEditAlbumTitle(album.title); // Thiết lập giá trị hiện tại của tiêu đề vào state
    setSelectedAlbumId(album.albumId); // Gán albumId của album cần chỉnh sửa
    setShowEditAlbumModal(true); // Hiển thị modal chỉnh sửa
  };
  ...
  const handleUpdateAlbum = () => {
    ...
    if (isValid) {
      ...
      axios
        .patch(`http://localhost:9999/albums/${selectedAlbumId}`, editAlbum)
        .then(() => {
  ...
  const handleDelAlbum = () => {
    const disableAlbum = async () => {
      try {
        await axios.patch(`http://localhost:9999/albums/${selectedAlbumId}`, {
          isActive: false,
        });
        // Cập nhật lại danh sách albums sau khi xóa
        setAlbums(albums.filter((album) => album.albumId !== selectedAlbumId));
  ...
  const handleAddPhoto = () => {
    ...
    axios
      .post("http://localhost:9999/photos", newPhotoData)
      .then(() => {
                ...
                >
                  <ul className="d-flex mb-0" style={{ listStyle: "none" }}>
                    ...
                    <li className="ps-2">
                      <Button
                        variant="danger"
                        onClick={() => setShowDelAlbumModeal(true)}
                      >
  ...
  return (
    <Container fluid>
      <Row style={{ marginTop: "20px" }}>
        ...
        <Col md={9}>{user ? renderContent() : <p>Loading...</p>}</Col>
      </Row>
      ...
      <Modal show={showAlbumModal} onHide={() => setShowAlbumModal(false)}>
        ...
        <Modal.Body>
          <Form.Group>
            ...
            />
            {newAlbumTitleErr && (
              <span style={{ color: "red" }}>{newAlbumTitleErr}</span>
            )}
          </Form.Group>
        </Modal.Body>
        <Modal.Footer>
  ...
  <Modal.Body>
    <Form.Group>
      ...
      <Form.Control
        type="text"
        value={editAlbumTitle} // Hiển thị giá trị hiện tại của tiêu đề album
        onChange={(e) => setEditAlbumTitle(e.target.value)} // Cập nhật state khi người dùng chỉnh sửa
      />
      {editAlbumTitleErr && (
        <span style={{ color: "red" }}>{editAlbumTitleErr}</span>
      )}
    </Form.Group>
  </Modal.Body>
  <Modal.Footer>
    ...
    <Button variant="primary" onClick={handleUpdateAlbum}>
      ...
      <Modal
        show={showDelAlbumModal}
        onHide={() => setShowDelAlbumModeal(false)}
      >
        <Modal.Header closeButton>
          <Modal.Title>Confirm delete album</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        <Form.Label>Are you sure want to delete this Album?</Form.Label>
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
          ...
          <Button variant="danger" onClick={handleDelAlbum}>
            Delete
          </Button>
        </Modal.Footer>
      </Modal>
